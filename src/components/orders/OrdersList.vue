<template>
  <div class="my-4 relative overflow-x-auto">
    ...
    <table class="w-full text-sm text-center rtl:text-right text-gray-500 dark:text-gray-400">
      ...
      <tbody>
        <tr
          v-for="order in orderStore.orders.slice(sliceStart, sliceEnd)"
          :key="order.uuid"
    ...
    <div class="w-full flex justify-between mt-4">
      ...
      <div class="flex">
        <span class="text-gray-500 mx-4 text-sm">
          ...
          - {{ currentPage * rowsPerPage }} of {{ orderStore.orders.length }}
        </span>
...
watch(rowsPerPage, async (newValue, oldValue) => {
  currentPage.value = Math.max(
    1,
    Math.min(
      currentPage.value > orderStore.orders.length / newValue
        ? orderStore.orders.length / newValue
        : currentPage.value,
      orderStore.orders.length / newValue
    )
  )
...
const next = () => {
  currentPage.value =
    (currentPage.value + 1) * rowsPerPage.value > orderStore.orders.length
      ? currentPage.value
      : currentPage.value + 1
  sliceEnd.value = rowsPerPage.value * currentPage.value
  sliceStart.value = sliceEnd.value - rowsPerPage.value
